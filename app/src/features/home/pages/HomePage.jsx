  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(async ([seasonsSnap, leaguesSnap, tournamentsSnap, matchesSnap]) => {
        ...
        const playerIds = [
          ...new Set(
            [...incoming, ...finished].flatMap((m) => [m.player1Id, m.player2Id].filter(Boolean)),
          ),
        ]
        if (playerIds.length > 0) {
          ...
          if (linkedUids.length > 0) {
            const userResults = await Promise.allSettled(
              linkedUids.map((uid) => getDoc(doc(db, 'users', uid))),
            )
            const userMap = {}
        ...
        const visibleMatches = [...incoming, ...finished].filter(
          (m) => m.competitionType && m.competitionId && m.roundId,
        )
        const uniqueRounds = [
          ...new Map(
            visibleMatches.map((m) => [`${m.competitionType}|${m.competitionId}|${m.roundId}`, m]),
          ).values(),
        ]
        if (uniqueRounds.length > 0) {
          ...
          const roundDocs = await Promise.all(
            uniqueRounds.map((m) =>
              getDoc(doc(db, `${cType(m)}/${m.competitionId}/rounds/${m.roundId}`)),
            ),
          )
          const groupIds = [
            ...new Set(
              roundDocs
                .filter((d) => d.exists() && d.data().groupId)
                .map((d) => ({
                  competitionId: d.ref.parent.parent.id,
                  groupId: d.data().groupId,
                  cTypePath: d.ref.parent.parent.parent.id,
                })),
            ),
          ]
          const groupDocs = await Promise.all(
            groupIds.map((g) =>
              getDoc(doc(db, `${g.cTypePath}/${g.competitionId}/groups/${g.groupId}`)),
            ),
          )
          const groupNameById = {}
          ...
          for (const d of roundDocs) {
            if (d.exists() && d.data().groupId)
              roundGroupMap[d.id] = groupNameById[d.data().groupId]
          }
        ...
        <section>
          ...
          {competitionsLoading ? (
            ...
            <div
              ...
              {...competitionsScrollHandlers}
              className="flex gap-4 overflow-x-auto pb-2 scrollbar-none cursor-grab"
            >
              {orderedSeasonIds
                .flatMap((sid) => {
          ...
          {loading ? (
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              {incomingMatches.map((match) => {
                ...
                const href =
                  cPath && match.competitionId && match.roundId
                    ? `/${cPath}/${match.competitionId}/rounds/${match.roundId}/matches/${match.id}`
                    : null
                const card = (
                  <MatchCard
                    key={match.id}
                    match={toIncomingMatchCardProps(
                      match,
                      playerById,
                      competitionNameById,
                      groupNameByRoundId,
