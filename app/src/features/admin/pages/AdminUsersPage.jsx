...
import { SectionTitle, Button, Loader, Badge, Alert, Select, Container } from '../../../ui/index.js'
...
export default function AdminUsersPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const [data, players] = await Promise.all([getAllUsers(), playersRepository.getAll()])
        setUsers(data)
        const initial = {}
        data.forEach((u) => {
          initial[u.id] = u.role || 'player'
        })
        setPendingRoles(initial)
        const map = {}
        players.forEach((p) => {
          if (p.authUid) map[p.authUid] = p
        })
        setLinkedByUid(map)
      } catch (err) {
        console.error(err)
