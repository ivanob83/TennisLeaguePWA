...
export default function LeagueEditPage() {
  const { leagueId } = useParams()
  const navigate = useNavigate()
  const { showToast } = useToast()
  const { data: league, loading: leagueLoading } = useFirestoreDoc('leagues', leagueId)
  ...
  useEffect(() => {
    if (league && !form) {
      setForm({
        ...
        numGroups: league.numGroups != null ? String(league.numGroups) : '',
        playersPerGroup: league.playersPerGroup != null ? String(league.playersPerGroup) : '',
        rules: league.rules || '',
      })
      setImageUrls(league.imageUrls || null)
    }
  }, [league, form])
  ...
  async function handleImageCrop(imageSrc, croppedAreaPixels) {
    ...
    setImageError(null)
    try {
      const urls = await uploadCompetitionImage(leagueId, imageSrc, croppedAreaPixels)
      setImageUrls(urls)
    } catch (err) {
      console.error(err)
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (result === 'timeout') {
        updatePromise.catch(() => {})
        showToast({ title: 'League updated', message: 'Saving in background.', variant: 'info' })
      } else {
        showToast({ title: 'League updated', message: 'Changes saved.', variant: 'success' })
      }
      setSubmitting(false)
      navigate(`/leagues/${leagueId}`)
      return
    } catch {
      setServerError('Failed to update league. Please try again.')
    }
    setSubmitting(false)
  }
  ...
  return (
    <AppLayout>
      <Container className="py-8">
        <div className="mb-2">
          <Button variant="ghost" size="sm" onClick={() => navigate(`/leagues/${leagueId}`)}>
            ← Back to League
          </Button>
        </div>
