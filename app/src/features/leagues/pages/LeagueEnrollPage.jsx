...
export default function LeagueEnrollPage() {
  ...
  const isGroupFormat =
    league?.format === 'round_robin' || league?.format === 'round_robin_knockout'

  return (
    <AppLayout>
      <div className="mx-auto max-w-2xl px-6 py-section">
        <div className="mb-2">
          <Button variant="ghost" size="sm" onClick={() => navigate('/leagues')}>
            ← Back to Leagues
          </Button>
        </div>
        {loading ? (
              ...
              <Card>
                <div className="space-y-3 text-sm">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="font-medium text-text">{league?.seasonId || '—'}</span>
                  </div>
                  {isGroupFormat && (
                    <>
                      <div className="flex items-center justify-between">
                        <span className="text-text-light">Groups</span>
                        <span className="font-medium text-text">{league?.numGroups || 0}</span>
                      </div>
                      <div className="flex items-center justify-between">
