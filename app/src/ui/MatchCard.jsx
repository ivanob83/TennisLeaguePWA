...
function normalizeStatus(status, scheduledTimestamp) {
  ...
  if (status === 'scheduled' && scheduledTimestamp) {
    const now = Date.now()
    const t = typeof scheduledTimestamp === 'number' ? scheduledTimestamp : new Date(scheduledTimestamp).getTime()
      ...
      <CardHeader className="mb-2">
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="flex shrink-0 flex-col items-end">
            ...
            {scheduleInfo && <span className="mt-0.5 text-right text-[14px] text-[#808080]">{scheduleInfo}</span>}
          </div>
        </div>
      </CardHeader>
                ...
                <div className="flex items-center">
                  ...
                  <div className="flex min-w-0 grow items-center gap-2">
                    ...
                    {ui.showWinnerIcon && isWinner ? (
                      <svg className="h-3.5 w-3.5 shrink-0 text-black" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="3" aria-hidden="true">
        ...
        <CardFooter className="mt-3 flex items-center justify-between border-t border-slate-200 pt-3">
          <div>
            {onAction && actionLabel ? (
              <Button size="sm" variant="success" onClick={onAction}>{actionLabel}</Button>
            ) : null}
          </div>
          {detailHref ? (
            <Link to={detailHref}>
