...
### Player kontakt — out of scope

Telefon se NE čuva na `players` doc-u. Admin već ima Viber/WhatsApp grupu sa svim igračima — kontakt postoji van sistema. Posledice za UI:

- Nema `phone` polja na player formama.
- Nema `viber://chat?number=` deep link-a (zahteva broj).
- Tab 2 dugme "Send message" → kopira generisanu template poruku u clipboard (`navigator.clipboard.writeText`) + toast "Poruka kopirana, nalepi u Viber". Admin sam otvara chat i nalepljuje.
...
- "Copy msg for X/Y" — generiše template poruku (vidi "Template poruke") i kopira u clipboard preko `navigator.clipboard.writeText`. Toast "Poruka kopirana". Admin sam otvara Viber/WhatsApp i nalepljuje.
...
export async function computeSuggestions({ competitionType, competitionId, fromDate, toDate }) {
  ...
  const allMatches = matchLists
    .flat()
    .filter((m) => m.status === 'not_scheduled' && m.player1Id && m.player2Id)
...
## Odluke (potvrđeno 2026-05-04)

- **Refresh modela u Tab 2** — bez real-time `onSnapshot`. Lista se učitava na otvaranje taba + manuelno "Refresh" dugme. Razlog: admin gleda 1–2× dnevno, live update nije potreban.
- **Bez toast/push notifikacije** za nove sugestije. Admin sam odlučuje kad da pogleda.
- **"Show only this week" filter** default ON (već u spec-u, eksplicitno potvrđeno).
- **Telefon se NE čuva** na `players` doc-u. Razlog: admin već ima Viber grupu sa svim kontaktima van sistema. Posledice: nema `phone` polja, nema deep linka, "Send message" dugme = copy-to-clipboard sa template tekstom.
